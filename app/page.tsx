...
export default function Page() {
  ...
  // static /hero-hi-wide.jpg whenever Pexels returns no match, has no API key,
  // or errors. URL only — image bytes are streamed directly from Pexels.
  useEffect(() => {
    if (!beachData) {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/image-proxy?beach=${encodeURIComponent(beachData.beach_name)}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (cancelled) return;
